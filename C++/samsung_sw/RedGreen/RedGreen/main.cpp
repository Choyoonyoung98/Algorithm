...
void find(int posY, int posX, char color, bool isRedGreen) {
    int currentY = posY;
    int currentX = posX;
    ...
    if(isRedGreen == false) {
        ...
        for(int i=0; i<4; i++) {
            ...
            if(checkIfSameColorNormal(nextY, nextX, color) == true)find(nextY, nextX, color, false);
        ...
        for(int i=0; i<4; i++) {
            nextY = currentY + moveDir[i].y;
            nextX = currentX + moveDir[i].x;
            if(checkIfSameColorGreenRed(nextY, nextX, color) == true)find(nextY, nextX, color, true);
...
int main(int argc, const char * argv[]) {
    ...
    solution();
    cout<<normalPeopleCatchColor<<" "<<redGreenPeopleCatchColor;
    return 0;
}
