...
Dir moveDir[4] = { {-1, 0}, {1, 0}, {0, 1}, {0, -1} }; //N S E W 좌 하 상 우

int R, C, T;
int arr[MAX][MAX];
//반시계 방향 인덱스
int ccw[4] = { 2, 0, 3, 1 }; //상 좌 우 하
//시계 방향 인덱스
int cw[4] = { 2, 1, 3, 0 };
void circulate(int airX, int airY, int airX2, int airY2, int copyArr[MAX][MAX]) {
    //공기청정기
    //위쪽
    int y = airY;
    int x = airX + 1;
...
void spread() {
    ...
    for (int t = 0; t < T; t++)
    {
        ...
        for (int i = 0; i < R; i++)
            for (int j = 0; j < C; j++)
                copyArr[i][j] = arr[i][j];
        circulate(airX, airY, airX2, airY2,copyArr);
...
int main(void) {
    //cin은 scanf에 비해 2~3배 느리다. 하지만 아래 코드를 사용할 경우 scanf에 근접하게 할 수 있다.
    //동기화를 풀고 속도를 끌어올리는 방법, 버퍼 수를 줄인다.
