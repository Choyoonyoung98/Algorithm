...
int dfs(int index, bool include) {
    ...
    if(include) { //본인이 워크숍 가는 인원에 포함된다면, 해당 팀원 직원을 포함해도 되고, 안해도 된다
        ...
        for(int follower: graph[index]) {
            sum += min(dfs(follower, true), dfs(follower, false));
        }
        return cost = sum;
        ...
        for(int follower: graph[index]) {
            int case1 = dfs(follower, true);
            int case2 = dfs(follower, false);
        ...
        return cost = participationExist ? sum : sum + diff;
    }
    return -1;
}
int solution(vector<int> sales, vector<vector<int>> links) {
    ...
    return answer = min(dfs(1, false), dfs(1, true));
}
